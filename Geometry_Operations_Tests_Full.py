...
def Length(bar):
    ...
    bar_vector = BarNodeToVector(bar, bar_node)
    # find the length of vector
    length = VectorTwoNorm(bar_vector)
...
def FindSharedNode(bar_1,bar_2):
    # Case 1: both bars start at the same node
    if(bar_1.init_node == bar_2.init_node):
        return bar_1.init_node
    # Case 2: bar_1 starts where bar_2 ends
    elif(bar_1.init_node == bar_2.end_node):
        return bar_1.init_node
    # Case 3: bar_1 ends where bar_2 starts
    elif(bar_1.end_node == bar_2.init_node):
        return bar_1.end_node
    # Case 4: both bars end at the same node
...
def BarsToVectors(bar_1,bar_2):
    # Find the node that both bars share
    shared_node = FindSharedNode(bar_1, bar_2)
    # Convert each bar into a vector that starts at the shared node and points away along the bar
    v1 = BarNodeToVector(bar_1, shared_node)
    v2 = BarNodeToVector(bar_2, shared_node)
...
def CosineBars(local_x_bar,other_bar):
    # Convert both bars into vectors pointing away from the shared node local_x_vec corresponds to the local x-axis direction
   local_x_vec, other_vec = BarsToVectors(local_x_bar, other_bar)
   # Compute the cosine of the angle between the two vectors using the CosineVectors helper function
   cos_theta = CosineVectors(local_x_vec, other_vec)
   #Return the cosine of the angle
   return cos_theta

# Sine of angle from local x bar to the other bar
def SineBars(local_x_bar,other_bar):
    # Convert both bars into vectors pointing away from the shared node local_x_vec is the local x-axis direction
    local_x_vec, other_vec = BarsToVectors(local_x_bar, other_bar)
    # Compute the sine of the angle between the two vectors using the SineVectors helper function
    sin_theta = SineVectors(local_x_vec, other_vec)
